...
class ControllerToolUpdate extends Controller {
    public function index() {
        ...
        $data['version'] = BITSTORE_BRASIL;
        $data['update'] = true;

        $curl = curl_init('https://api.github.com/repos/bitstore-brasil/bitstore-brasil/releases');

        curl_setopt($curl, CURLOPT_USERAGENT, 'Bitstore Brasil ' . BITSTORE_BRASIL);
        ...
        if ($response) {
            ...
            $release_local = str_replace("v", "", BITSTORE_BRASIL);
    ...
    public function download() {
        ...
        if (!$json && isset($this->session->data['version'])) {
            ...
            $file = DIR_DOWNLOAD . 'bitstore-brasil.zip';
            if (is_file($file)) {
                @unlink($file);
            }

            $handle = fopen(DIR_DOWNLOAD . 'bitstore-brasil.zip', 'w');

            $curl = curl_init('https://github.com/bitstore-brasil/bitstore-brasil/releases/download/'.$this->session->data['version'].'/bitstore-brasil.zip');
    ...
    public function unzip() {
        ...
        if (!$json) {
            ...
            if ($zip->open($file)) {
                $zip->extractTo(DIR_DOWNLOAD . 'bitstore-brasil/');
                $zip->close();
    ...
    public function move() {
        ...
        $directory = DIR_DOWNLOAD . 'bitstore-brasil/';
    ...
    private function remove() {
        $directory = DIR_DOWNLOAD . 'bitstore-brasil/install/';
        ...
        foreach ($files as $file) {
            $file = DIR_DOWNLOAD . 'bitstore-brasil/'.$file;
            if (is_file($file)) {
                @unlink($file);
            }
        }
